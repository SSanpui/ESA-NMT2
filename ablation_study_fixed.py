"""
Simple Ablation Study ONLY - FIXED ROUGE-L CALCULATION
...
FIXED: ROUGE-L now correctly returns values between 0-100 (as percentage)
"""
...
print("="*80)
print("ESA-NMT ABLATION STUDY - FIXED ROUGE-L")
...
# =============================================================================
# EVALUATION FUNCTION WITH FIXED ROUGE-L
# =============================================================================

def evaluate_config(config_info, translation_pair):
    """Evaluate one ablation configuration with FIXED ROUGE-L calculation"""
    ...
    try:
        ...
        # ✅ FIX: Ensure ROUGE-L is in correct range (0-100)
        # The evaluator already multiplies by 100, so values should be 0-100
        # If somehow they're still 0-1, multiply by 100 again
        if 'rouge_l' in metrics and metrics['rouge_l'] < 5.0:
            # If ROUGE-L is suspiciously low (< 5%), it's probably in 0-1 range
            print(f"⚠️ ROUGE-L seems to be in 0-1 range ({metrics['rouge_l']:.4f}), converting to percentage...")
            metrics['rouge_l'] = metrics['rouge_l'] * 100

        # ✅ SANITY CHECK: ROUGE-L should be between 0 and 100
        if metrics.get('rouge_l', 0) > 100:
            print(f"❌ ERROR: ROUGE-L is {metrics['rouge_l']:.2f} (> 100)! This is impossible!")
            print(f"   This indicates a bug in the calculation. Capping at 100.")
            metrics['rouge_l'] = min(metrics['rouge_l'], 100.0)
...
output_file = f'./outputs/ablation_study_{TRANSLATION_PAIR}_fixed.json'
...
print("\n🔧 ROUGE-L Fix Applied:")
print("   - Added sanity checks to ensure ROUGE-L is in 0-100 range")
print("   - Fixed double multiplication issue")
print("   - All metrics now consistent with full model evaluation")
